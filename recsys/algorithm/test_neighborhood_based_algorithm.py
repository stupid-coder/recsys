...
class TestItemBasedAlgorithm(object):
    def test_mean_center_predictor_algo(self):
        algo = ItemBasedNeighborhoodAlgorithm(NeighborhoodBasedConfig(sim_config=SimilarityConfig(name="cosine"), topk=2, sim_threshold=None, predictor_config=PredictorConfig(name="norm")))
        algo.fit(test_rating_data)
        rating_hat = algo.predict(None)
